...
export default function LanguageMain() {
  ...
  const handleLanguageSelection = async (selectedLanguage: string) => {
    ...
    await AsyncStorage.setItem("foodexpress-language", selectedLanguage);
  };
  const handleSetCurrentLanguage = async () => {
    try {
      const lng = await AsyncStorage.getItem("foodexpress-language");
      if (lng) {
        setIsSelected(lng);
      }
    } catch (error) {
  ...
  const handleSubmission = async () => {
    try {
      ...
      await AsyncStorage.setItem("foodexpress-language", isSelected);
