...
const LanguageModal = ({ modalVisible, setModalVisible, currentTheme, showCrossButton, dontClose }) => {
  ...
  async function determineInitialLanguage() {
    try {
      ...
      const storedLanguageCode = await AsyncStorage.getItem('foodexpress-language')
  ...
  async function onSelectedLanguage() {
    try {
      ...
      await AsyncStorage.setItem('foodexpress-language', languageCode)
      await AsyncStorage.setItem('foodexpress-language-name', languageVal)
